...
doc <- body_add_fpar(doc, fpar( ftext("as.character(text1)", prop = text_style), fp_p = par_style ) )
doc <- body_add_par(doc, " ")
...
doc <- body_add_flextable(doc,set_table_properties(flextable(table2), width = 1, layout = "autofit"))
doc2 <- print(doc, target = "My report.docx")
